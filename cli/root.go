// Package cli builds the gapminder command tree on top of the gapminder library.
package cli
...
// App holds shared state for all subcommands.
type App struct {
	client *gapminder.Client
	cfg    gapminder.Config
...
func Root() *cobra.Command {
	app := &App{cfg: gapminder.DefaultConfig()}

	root := &cobra.Command{
		Use:           "gapminder",
		Short:         "Explore Gapminder global development data",
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = gapminder.NewClient(a.cfg)
	return nil
}

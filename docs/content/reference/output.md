...
describes what users get. Pick a format with `-o`, or let gapminder choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
gapminder <command> -o table   # aligned columns for reading
gapminder <command> -o jsonl   # one JSON object per line, for piping
gapminder <command> -o json    # a single JSON array
gapminder <command> -o csv     # spreadsheet friendly
gapminder <command> -o tsv     # tab-separated
gapminder <command> -o url     # just the URL column
gapminder <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
gapminder <command> --fields id,title,url
```
...
```bash
gapminder <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
gapminder <command>            # a table, because this is a terminal
gapminder <command> | wc -l    # JSONL, because this is a pipe
```
